...
class LevitatorArray:
    ...
    The whole geometry can be uniformly rescaled with the `scale` argument,
    e.g. scale=1.6 for a build using 16 mm transducers instead of the
    standard 10 mm (piston radius 5 mm x 1.6 = 8 mm).
    """
    ...
    def __init__(self, curvature_radius=0.090, piston_radius=5.0e-3,
                 gap=0.0, freq=FREQ_HZ, scale=1.0):
        """
        curvature_radius : spherical cap radius [m] (concavity of each dish),
                           *before* scaling.
        piston_radius    : transducer piston radius [m] (default 5 mm = 10 mm
                           diameter), *before* scaling.
        gap              : extra axial separation [m] added between the two
                           caps (0 = caps share the same focal sphere),
                           *before* scaling.
        freq             : operating frequency [Hz].  Independent of `scale`;
                           a 16 mm 40 kHz transducer still runs at 40 kHz.
        scale            : isotropic linear scale factor applied to the whole
                           geometry (curvature radius, piston radius, gap and
                           all transducer positions).  A 1.6x build with
                           16 mm transducers uses scale=1.6 (5 mm base piston
                           radius x 1.6 = 8 mm = 16 mm diameter).  Ring polar
                           angles and transducer counts are NOT scaled.
        """
        self.scale = float(scale)
        self.Rc = float(curvature_radius) * self.scale
        self.piston_radius = float(piston_radius) * self.scale
        self.gap = float(gap) * self.scale
    ...
    def _build_shell(self, sign):
        ...
        shell = []
        for ring_index, (ang, n) in enumerate(zip(self._RING_ANGLES, self._RING_COUNTS)):
            ...
            # Stagger each ring 10 deg clockwise (viewed from the shell's own
            # outward axis) relative to the previous inner ring.  ring 0 is
            # the innermost ring (smallest polar angle), so each outer ring
            # is rotated by an additional -10 deg in its local azimuth.
            ring_azimuth_deg = -10.0 * ring_index
            offset = np.radians(ring_azimuth_deg)
            shell.extend(_ring(center, axis, n, r, z, ring_phase_offset=offset))
        for tr in shell:
...
def demo(scale=1.0, graph=60):
    """Run the simulation and save example figures to ../outputs.

    scale : isotropic geometry scale factor (1.0 = standard 10 mm
            transducers; 1.6 = 16 mm transducers, i.e. a 1.6x TinyLev).
    graph : +- graph scaling (from -n to n)
    ...
    mm = 1e-3
    lev = LevitatorArray(curvature_radius=0.090, piston_radius=5 * mm, scale=scale)
    field = AcousticField(lev)
    ctrl = PhaseControl(lev)

    print(f"Frequency     : {lev.freq/1e3:.1f} kHz")
    print(f"Wavelength    : {lev.lam*1e3:.2f} mm")
    print(f"Scale         : {lev.scale:.2f}x  (piston diameter {2*lev.piston_radius*1e3:.1f} mm)")
    ...
    X, Y, Z = _mesh_axis(-graph * mm, graph * mm, 400, -graph * mm, graph * mm, 400, y=0.0)
    ...
    for tr in lev.transducers:
        ...
        fig.savefig(os.path.join(outdir, "06_geometry.png"), dpi=120)
        plt.close(fig)
